...
def has_request_arg(fn):
    ...
    for name, param in params.items():
        ...
        if found and (param.kind != inspect.Parameter.VAR_POSITIONAL and
            ...
            raise ValueError(f'request parameter must be the last named parameter in function: {fn.__name__}{str(sig)}')
    return found
    ...
    async def __call__(self, request):
        ...
        if self._has_var_kw_arg or self._has_named_kw_args:
            if request.method == 'POST':
                if not request.content_type:
                    return web.HTTPBadRequest(body=b'Missing Content-Type.')
                ...
                if ct.startswith('application/json'):
                    ...
                    return web.HTTPBadRequest(body=f'Unsupported Content-Type: {request.content_type}'.encode())
        ...
        if self._required_kw_args:
            for name in self._required_kw_args:
                if not name in kw:
                    return web.HTTPBadRequest(body=f'Missing argument: {name}'.encode())
        logging.info(f'call with args: {kw}')
        try:
...
def add_route(app, fn):
    ...
    logging.info(f'add route {method} {path} => {fn.__name__}({params})')
