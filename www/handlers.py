...
def text2html(text):
    ...
    lines = [f'<p>{escape(s)}</p>' for s in text.split('\n') if s.strip() != '']
    return ''.join(lines)
...
async def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uid, name=name.strip(), email=email, passwd=hashlib.sha1(sha1_passwd.encode()).hexdigest(), admin=0,
